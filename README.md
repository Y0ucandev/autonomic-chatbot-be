...
Searches the message history for a specific user by a text query.

- **Query parameters:**
  - `user_id` (integer, required) — ID of the user whose messages are retrieved
  - `query`  (string, required)  — Text or phrase to search for in the user's messages
  - `limit` (integer, optional, default: 20) — Number of messages to retrieve (min 1, max 100)
  - `offset_id` (integer, optional, default: 0) — Offset ID for pagination
...
### `GET /user-facts/{user_id}`

Retrieves all known facts associated with a specific user.

- **Path parameters:**
  - `user_id` (integer, required) —  ID of the user whose facts are retrieved
